...
SRC="$LOG_DIR/route_log_thompson.jsonl"
...
# Bring up router in Thompson Sampling mode
docker compose \
  -f "$ROOT_DIR/docker-compose.yml" \
  -f "$ROOT_DIR/docker-compose.thompson.yml" \
  up -d --build rl_router

echo "[TS] reward config:"
curl -s http://localhost:8080/health || true
echo

echo "[TS] warm-up 3 requests…"
for i in 1 2 3; do
...
echo "[TS] running dataset $PROMPTS_FILE with tag=$TAG"
...
if [[ -n "$LOG_LABEL" ]]; then
  DST="$LOG_DIR/route_log_thompson_${TAG}_${LOG_LABEL}_${TS}.jsonl"
else
  DST="$LOG_DIR/route_log_thompson_${TAG}_${TS}.jsonl"
fi
